...
df = pd.read_excel("out2_port.xlsx")
...
for name, mini_df in df.groupby("Closest Port"):
    color = choice(colors)
    station_lon = mini_df.iloc[0]["Closest Port Longitude"]
    station_lat = mini_df.iloc[0]["Closest Port Latitude"]
    ...
    fig.add_trace(
        go.Scattermapbox(
            mode="markers",
            marker=dict(size=10, symbol=["harbor"]),
            lon=[station_lon],
            lat=[station_lat],
        )
    )
