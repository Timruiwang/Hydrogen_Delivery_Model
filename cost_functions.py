...
def LH_storage_terminal(DailyTrans):
    ...
    RES = {"CAPEX": 0, "elec_cost": 0, "lab_cost": 0, "elec_demand": 0}
...
def GH_truck(Distance, DT):
    ...
    OPEX_fre_GHT = Num_truck_gh * proj_days * RTD_GHT * fuel_econ_avg * U_fuel
...
def LH_truck(Distance, DT):
    ...
    fuel_econ_avg = 2.6  # KM/L Average fuel economy
    U_fuel = 2  # $ Cost of fuel in 2010[7]
    OPEX_fre_LHT = Num_truck_lh * proj_days * RTD_LHT * fuel_econ_avg * U_fuel
...
def GH_pipeline(Distance, DT):
    ...
    Dia_inches = 1  # 3.880*(DT/(roh_LH2*1000)*P_b*T_f*L*Z*mu*T_b**7*(P_in_pip**2-P_out_pip**2)**7)**0.25/(T_b**2*(P_in_pip**2-P_out_pip**2)**7) #Inches required pipeline diameter
    ...
    CAPEX_eq_PIP = (CAPEX_row+CAPEX_mat + CAPEX_lab + CAPEX_misc)/1 #1.17390795349974E-8*DT*L*0.01#
